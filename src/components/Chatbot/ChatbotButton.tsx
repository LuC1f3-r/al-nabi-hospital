...
const ChatbotButton: React.FC<ChatbotButtonProps> = ({ isOpen, onClick }) => {
  return (
    <button className="button" onClick={onClick}>
      ...
      {/* Default state */}
      <div className="state state--default" style={{ display: isOpen ? "none" : "flex" }}>
        <div className="icon">
          {/* ✈️ Plane SVG */}
          <svg
        ...
        <p>
          {["S","e","n","d"," ","M","e","s","s","a","g","e"].map((letter, i) => (
            <span key={i} style={{ ["--i" as any]: i }}>{letter}</span>
          ))}
        </p>
      </div>

      {/* Sent state */}
      <div className="state state--sent" style={{ display: isOpen ? "flex" : "none" }}>
        <div className="icon">
          <svg
            stroke="black"
            strokeWidth="0.5px"
        ...
        <p>
          {["S","e","n","t","!"].map((letter, i) => (
            <span key={i} style={{ ["--i" as any]: i+5 }}>{letter}</span>
          ))}
        </p>
      </div>
    </button>
